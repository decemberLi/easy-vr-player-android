...
└── app/
    ...
    └── src/main/
        ...
        └── cpp/
            ...
            ├── control_panel.{h,cpp}      浮动控制面板 + 射线/凝视命中
...
- 手柄射线命中由 OpenXR 的 aim pose 提供；当前回退到头部凝视命中（v0 实现）。
...
| 控制面板射线偏 | v0 用头部凝视，扭头点选；后续可切到控制器 aim pose（代码已埋好，把 `HitTestGaze` 换成 `HitTestRay(aim_pose)`） |
...
- 控制器射线代替头部凝视（OpenXR 已建好 `aim_pose` 动作和动作空间，只要把 `HitTestRay(aimPose)` 取代 `HitTestGaze`）
